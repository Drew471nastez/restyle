...
import { ArrowRight, Shield, Truck, Banknote, Sparkles, ChevronRight } from 'lucide-react';
...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Banner */}
      <section className="relative w-full overflow-hidden bg-gradient-to-br from-teal-600 via-teal-500 to-emerald-500">
        ...
        <div className="relative mx-auto max-w-[1440px] px-4 lg:px-8 py-12 sm:py-16 lg:py-20">
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <Link
                href="/browse"
                className="inline-flex items-center justify-center gap-2 h-12 px-8 bg-white text-teal-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition shadow-lg shadow-black/10"
              >
                {t('nav.browse')}
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 sm:grid sm:grid-cols-3 lg:grid-cols-6 sm:overflow-visible">
            {categories.map((cat) => (
              <Link
                ...
                className="group shrink-0 w-36 sm:w-auto"
              >
        ...
        <div className="mx-auto max-w-[1440px] px-4 lg:px-8">
          ...
          {listings.length > 0 ? (
            ...
            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <Link
                href="/login"
                className="inline-flex items-center gap-2 h-11 px-7 bg-teal-500 text-white rounded-xl text-sm font-semibold hover:bg-teal-600 transition shadow-sm"
              >
                Start selling
                <ArrowRight className="h-4 w-4" />
              </Link>
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
            {['Nike', 'Zara', 'H&M', 'Adidas', 'Gucci', 'Prada', 'Mango', 'Pull&Bear', 'Levi\'s', 'New Balance', 'Balenciaga', 'Bershka'].map((brand) => (
              <Link
                ...
                className="shrink-0 flex items-center justify-center h-12 px-6 bg-gray-50 rounded-xl text-sm font-medium text-gray-700 hover:bg-teal-50 hover:text-teal-700 border border-gray-200 hover:border-teal-200 transition whitespace-nowrap"
              >
                {brand}
              </Link>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              ...
              <div key={item.step} className="flex flex-col items-center text-center bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100">
                ...
                <div className="w-8 h-8 bg-teal-500 rounded-full flex items-center justify-center text-white font-bold text-sm mb-3">
                  {item.step}
                </div>
      ...
      {/* CTA Banner */}
      <section className="py-14 sm:py-16 bg-gradient-to-br from-teal-600 to-emerald-600">
        <div className="mx-auto max-w-[1440px] px-4 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/signup"
              className="inline-flex items-center justify-center gap-2 h-12 px-8 bg-white text-teal-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition shadow-lg"
            >
              {t('common.signup')}
              <ArrowRight className="h-4 w-4" />
            </Link>
